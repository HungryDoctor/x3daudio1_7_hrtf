	...
	, m_on_destroy(on_destroy)
{
	...
	HRESULT result;
	if (SUCCEEDED(result = original_xaudio->CreateSourceVoice(&original_voice, pSourceFormat, Flags, MaxFrequencyRatio, pCallback, pSendList ? &originalSendList : 0, nullptr)))
	{
		m_original = original_voice;
		m_impl.reset(new XAudio2VoiceProxy(L"XAudio2SourceVoiceProxy", m_sound3d_registry, m_voice_mapper, m_original, pSourceFormat->nChannels, this));
